...
class TestProductModel(unittest.TestCase):
    ...
    def test_create_product_with_ascii_art_and_quantity(self):
        """Test creating a new product with ASCII art and quantity."""
        Product.create_product("Test Product with ASCII", 99.99, "A test product with ASCII art", self.user_id, quantity=10, db=self.db, ascii_art=self.ascii_art)
    ...
    def test_get_all_products_with_ascii_art_and_quantity(self):
        """Test fetching all products, including ASCII art and quantity."""
        Product.create_product("Product 1", 49.99, "Description 1", self.user_id, quantity=5, db=self.db, ascii_art=self.ascii_art)
        Product.create_product("Product 2", 149.99, "Description 2", self.user_id, quantity=3, db=self.db, ascii_art=self.ascii_art)
        ...
        self.assertEqual(products[0][6], 5)  
        self.assertEqual(products[1][6], 3)  

    def test_get_product_by_id_with_ascii_art_and_quantity(self):
        """Test fetching a product by its ID, including ASCII art and quantity."""
        Product.create_product("Product ID Test", 59.99, "Description for ID", self.user_id, quantity=7, db=self.db, ascii_art=self.ascii_art)
        ...
        self.assertEqual(product[5], self.ascii_art)
        self.assertEqual(product[6], 7)  

    def test_update_product_with_ascii_art_and_quantity(self):
        """Test updating an existing product, including the ASCII art and quantity."""
        Product.create_product("Update Test", 79.99, "Before update", self.user_id, quantity=2, db=self.db, ascii_art=self.ascii_art)
        ...
        Product.update_product(product_id, name="Updated Product", price=89.99, description="After update", quantity=5, ascii_art=new_ascii_art, db=self.db)
        ...
        self.assertEqual(product[6], 5) 

    def test_delete_product_with_ascii_art(self):
        """Test deleting an existing product with ASCII art."""
        Product.create_product("Delete Test", 199.99, "Delete this product", self.user_id, quantity=8, db=self.db, ascii_art=self.ascii_art)
